...
InternalNode::InternalNode(Inmem *inmem, Comparator *cmp, kvssd::KVSSD *kvd, InternalNode *p, int fanout, int level) 
: inmem_(inmem), cmp_(cmp), kvd_(kvd), parent_(p), level_(level), key_array_(NULL), dirty_(true), fanout_(fanout),
...
InternalNode::InternalNode(Inmem *inmem, Comparator *cmp, kvssd::KVSSD *kvd, InternalNode *p, int fanout, int level, char *buf, uint32_t size)
: inmem_(inmem), cmp_(cmp), kvd_(kvd), parent_(p), level_(level), key_array_(NULL), dirty_(false), fanout_(fanout),
  sorted_run_(custom_cmp(cmp)) {
   //4B: number of entries 
...
void InternalNode::Flush() {
   ...
   // kvd_->kv_store(&inner_key, &inner_val);
    inmem_->leaf_run_.insert(inner_key, inner_val, cmp_);
    // 4, clean up
    free(buf);
}
...
LeafNode::LeafNode(Inmem* inmem, Comparator *cmp, kvssd::KVSSD *kvd, InternalNode *p) 
:inmem_(inmem), cmp_(cmp), kvd_(kvd), parent_(p), buf_(NULL), entries_(0), sorted_run_(custom_cmp(cmp)) {}

//buf is the same as value format in SSD  
//buf to in memory leaf node 
LeafNode::LeafNode(Inmem* inmem, Comparator *cmp, kvssd::KVSSD *kvd, InternalNode *p, char *buf, uint32_t size) 
: inmem_(inmem), cmp_(cmp), kvd_(kvd), parent_(p), buf_(buf), entries_(0), sorted_run_(custom_cmp(cmp)) {
...
void LeafNode::Flush() {
    ...
    *(uint32_t *)p = next_leaf_.size();
    ...
    inmem_->leaf_run_.insert(&leaf_key, &leaf_val, cmp_);
    // 4, clean up
    free(buf);
}
...
: cmp_(cmp), kvd_(kvd), cache_size_(cache_size), level_(1), fanout_(fanout) {
    ...
    if (newDB) {
        root_ = new InternalNode(inmem_, cmp_, kvd_, NULL, fanout_, 0);
	...
	//modification
        *vbuf = (inmem_->internal_run_.get(&root_key_)).ToString();
        *vlen = (*vbuf).size();
...
void KVBplusTree::Append(Slice *key) {
	...
	//from root get to the leaf node 
		...
		*v_buf = (inmem_->internal_run_.get(io_key)).ToString();
		v_size = (*v_buf).size();
                node = new InternalNode(inmem_, cmp_, kvd_, node, fanout_, level_-i-2, v_buf, v_size);
                free(v_buf);
        ...
        int leaf_entries = node->GetChildEntries(&lower_key);
	    ...
	    v_buf = (inmem_->leaf_run_.at(leaf_key)).ToString();
	    inmem_->leaf_run_.erase(leaf_key);
	    v_size = v_buf.size();
            leaf_node = new LeafNode(inmem_,cmp_, kvd_, node, v_buf, v_size);
        ...
        if (leaf_split) {
            ...
            do {
                ...
                if (internal_split != NULL && node->Parent() == NULL) { // new root
                   ...
                   // std::string right_node_key = "level"+std::to_string(level)+node->FirstKey().ToString();
                    ...
                    root_ = new InternalNode(inmem_, cmp_, kvd_, NULL, fanout_, level+1);
		    ...
		    inmem_->erase(Slice(old_node_key));
		   //TODO
		    //internal split to Slice and insert  old_node key 
		    //node  to Slice and insert  new_node_key 
...
void KVBplusTree::Iterator::Seek(Slice *key) {
    ...
    for (int i = 0; i < level; i++) {
	    ...
	    std::string v_buf = (tree_->inmem_->internal_run_(io_key)).ToString();
		int v_size = v_buf.size();
            node = new InternalNode(tree_->inmem_, tree_->cmp_, kvd, node, tree_->fanout_, level-i-2, &v_buf, v_size);
            free(v_buf);
    ...
    std::string v_buf = (tree_->inmem_->leaf_run_.at(leaf_key)).ToString();
    int v_size = v_buf.size();

    leaf_ = new LeafNode(tree_->inmem_, tree_->cmp_, NULL, NULL, &v_buf, v_size);
    ...
    if (!leaf_->Valid(it_)) { // go to next leaf node
        ...
        v_buf = (tree_->inmem_->leaf_run_.at(next_leaf)).ToString();
        v_size = v_buf.size();
...
void KVBplusTree::Iterator::Next() {
    ...
    if (!leaf_->Valid(it_)) { // recursively traverse internal node 
        ...
        std::string v_buf = (tree_->inmem_->leaf_run_.at(next_leaf)).ToString();
        int v_size = v_buf.size();
        leaf_ = new LeafNode(tree_->inmem_, tree_->cmp_, NULL, NULL, &v_buf, v_size);
        it_ = leaf_->SeekToFirst();
    }
